...
import { criarRegistro, editarRegistro } from "@/utils/axiosService";
import { useEffect, useState } from "react";
import { SubmitHandler, useForm } from "react-hook-form";

function DetalhesSistema({ abrir, onFechar, dados }: PropriedadesDialogEdicao<SistemaNameSpace.EntidadeSistema>) {


    const [isSuccesso, setIsSuccesso] = useState(false);
    const { register, handleSubmit, formState: { errors }, setValue, watch, reset } = useForm<SistemaNameSpace.EntidadeSistema>();
    ...
    const editarSistema: SubmitHandler<SistemaNameSpace.EntidadeSistema> = async (dados) => {

        try {
            await editarRegistro<SistemaNameSpace.EntidadeSistema>({ data: dados }, 'editarSistema', dados.idSistema);
            setErrorMessage('');
            setIsSuccesso(true);
            setTimeout(() => {
                setIsSuccesso(false);
            ...
            setErrorMessage('Erro ao editar dados do sistema. Tente novamente.');
                    ...
                    <div className="bg-card rounded-lg shadow-md overflow-hidden h-full">
                        <div className="p-6 grid gap-4 h-full">
                            <div className="flex-1">
                                ...
                                <form className="grid gap-4 text-muted-foreground" onSubmit={handleSubmit(editarSistema)}>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <Select
                                            defaultValue={dados.sistemaAtivo}
                                            onValueChange={(dado) => {
...
export default DetalhesSistema; 